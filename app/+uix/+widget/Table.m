classdef Table < matlab.mixin.SetGet
    %  uix.widget.Table - Class definition for Table
    %   The Table object places a Java table control within a figure or
    %   container.
    %
    % Syntax:
    %   t = uix.widget.Table
    %   t = uix.widget.Table('Property','Value',...)
    %
    %  uix.widget.Table Properties:
    %
    ...
    %
    %  uix.widget.Table Example:
    %
    %   % Create the figure and display the table
    %   f = figure;
    %   t = uix.widget.Table('Parent',f);
    ...
    methods
        ...
        function obj = Table(varargin)
            %  uix.widget.Table - Constructor for Table
            % -------------------------------------------------------------------------
            % Abstract: Constructs a new Table object.
            %
            % Syntax:
            %           obj = uix.widget.Table('p1',v1,...)
            %
            % Inputs:
            %           Property-value pairs
            %
            % Outputs:
            %           obj - uix.widget.Table object
            %
            % Examples:
            %           hFig = figure;
            %           obj =  uix.widget.Table('Parent',hFig)
            %
...
%             uiextras.jTable.loadJavaCustomizations();
    ...
    methods % Public methods
        function setCellColor( obj , rowIndex , colIndex , color )
            %setCellColor Sets the background color for a cell
            %
            % setCellColor( obj , rowIndex , colIndex , color )
            %
            % Inputs
            %   rowIndex: row of cell to set.
            ...
            if rowIndex>size(obj.Data,1) || colIndex>size(obj.Data,2) || ...
            ...
            jColor = uix.utility.getJavaColor(color);
            obj.JTable.setCellColor(rowIndex,colIndex,jColor)
    ...
    methods (Access = private)
        function create(obj, ParamsToSet)
            ...
            if isstruct(ParamsToSet) && isfield(ParamsToSet,'Parent')
            ...
            if isstruct(ParamsToSet) && ~isempty(fieldnames(ParamsToSet))
                set(obj,ParamsToSet);
            end
    ...
    methods
        ...
        function refreshRenderers(obj)
            ...
            % renderers.
            rObj = uix.widget.TableRenderers.getRenderers();
    ...
    methods
        ...
        function set.ColumnFormat(obj, value)
            ...
            % Validate the input
            [StatusOk, Message] = uix.widget.TableRenderers.validateColumnFormat(value);
            if ~StatusOk
                error(Message);
            end
