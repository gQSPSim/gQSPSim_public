classdef Tree < matlab.mixin.SetGet
    % Tree - Class definition for Tree
    %   The Tree object places a tree control within a figure or
    %   container.
    %
    % Syntax:
    %   nObj = uix.widget.Tree
    %   nObj = uix.widget.Tree('Property','Value',...)
    %
    % Tree Properties:
    %
    ...
    %   tree
    %
    %   KeyPressedCallback - callback for a keyboard button press
    %
    ...
    %   f = figure;
    %   t = uix.widget.Tree('Parent',f,...
    %       'SelectionChangeFcn','disp(''SelectionChangeFcn triggered'')');
    %
    %   %% Create tree nodes
    %   Node1 = uix.widget.TreeNode('Name','Node_1','Parent',t.Root);
    %   Node1_1 = uix.widget.TreeNode('Name','Node_1_1','Parent',Node1);
    %   Node1_2 = uix.widget.TreeNode('Name','Node_1_2','Parent',Node1);
    %   Node2 = uix.widget.TreeNode('Name','Node_2','Parent',t.Root);
    %
    %   %% Set an icon
    ...
    %   t.Enable = 'on';
    %
    % See also: uix.widget.CheckboxTree, uix.widget.TreeNode,
    ...
    methods
        function tObj = Tree(varargin)
            ...
            %
            % Syntax:
            %           tObj = uix.widget.Tree('p1',v1,...)
            %
            % Inputs:
            %           Property-value pairs
            %
            % Outputs:
            %           tObj - uix.widget.Tree object
            %
            % Examples:
            %           hFig = figure;
            ...
            if verLessThan('matlab','8.2')
                p.addParamValue('FontAngle','normal'); %#ok<NVREPL>
                p.addParamValue('FontName','MS Sans Serif'); %#ok<NVREPL>
                p.addParamValue('FontSize',10); %#ok<NVREPL>
                p.addParamValue('FontWeight','normal'); %#ok<NVREPL>
                p.addParamValue('Parent',[]); %#ok<NVREPL>
                p.addParamValue('Units','normalized'); %#ok<NVREPL>
                p.addParamValue('Position',[0 0 1 1]); %#ok<NVREPL>
            else
    ...
    methods (Access = protected)
        ...
        function createTree(tObj)
            ...
            tObj.Root = uix.widget.TreeNode('Name','Root','Tree',tObj);
        ...
        function createTreeCustomizations(tObj)
            ...
            set(CbProps,'KeyPressedCallback',@(src,e)onKeyPressed(tObj,e))
...
%             set(CbProps,'MouseMovedCallback',@(src,e)onMouseMotion(tObj,e))
        ...
        function nObj = getNodeFromMouseEvent(tObj,e)
            ...
            if isempty(treePath)
                nObj  = uix.widget.TreeNode.empty(0,1);
            else
        ...
        function onKeyPressed(tObj,e)
            % Occurs when any button is pressed in the keyboard
            
            % Is there a custom KeyPressedCallback?
            if callbacksEnabled(tObj) && ~isempty(tObj.KeyPressedCallback)
                ...
                e1 = struct(...
                    'KeyPressed',keyCode,...
                    'SelectedNodes',tObj.SelectedNodes);
                hgfeval(tObj.KeyPressedCallback,tObj,e1);
                
            end %if ~isempty(tObj.KeyPressedCallback)
        ...
        function onNodeSelection(tObj,e)
            ...
            if callbacksEnabled(tObj) && ~isempty(tObj.SelectionChangeFcn)
                ...
                % selection
                p = e.getPaths;
                AddedNodes = uix.widget.TreeNode.empty(0,1);
                RemovedNodes = uix.widget.TreeNode.empty(0,1);
                for idx = 1:numel(p)
        ...
        function onNodeEdit(tObj,e)
            ...
            if callbacksEnabled(tObj) && ~isempty(tObj.NodeEditedCallback)
                ...
                c = e.getChildren;
                EditedNode = uix.widget.TreeNode.empty(0,1);
                for idx = 1:numel(c)
        ...
        function onNodeDND(tObj,e)
            ...
            if callbacksEnabled(tObj)
                ...
                SourceNode = uix.widget.TreeNode.empty(0,1);
    ...
    methods
        ...
        function set.SelectedNodes(tObj,value)
            ...
            elseif isa(value,'uix.widget.TreeNode')
                if isscalar(value)
