classdef main < handle
    ...
    methods (Access = public)
        function app = main
            ...
            tree.Position = [10 10 figurePosition(3)-20 figurePosition(4)-20];
            
            % For development purposes load a Session here
            Session = load('tests/baselines/CaseStudy_TMDD_complete/CaseStudy1_TMDD.qsp.mat');            
            app.Session = Session.Session;
            app.createSession(tree); 
            ...
            if false
                ...
                pOptions.Region = 'left';
                pOptions.Title = 'Task';
                p1 = matlab.ui.internal.FigurePanel(pOptions);
                app.container.addPanel(p1);
                app.simulationTask = SimulationTask(p1);
                constructDataFittingTask(app);
                app.populationTask = SimulationTask(p1, 6, true); % last input controls popSim functionality.
                
                % Plot Settings
                pOptions.Region = 'right';
                pOptions.Title = 'Plot Settings';
                p1 = matlab.ui.internal.FigurePanel(pOptions);
                app.container.addPanel(p1);
                %constructPlotSettings(app);
                app.plotSettings = PlotSettings(p1);
                ...
                % Profile Notes
                pOptions.Region = 'bottom';
                pOptions.Title = 'Profile Notes';
                p1 = matlab.ui.internal.FigurePanel(pOptions);
                app.container.addPanel(p1);
                app.simulationResultsManager = SimulationResultsManager(p1);
        ...
        function createSession(app, tree)            
            sessionNode = app.createTreeNode(tree, app.Session, app.Session.SessionName, 'folder_24.png', 'Session');
            
            buildingBlocksNode = app.createTreeNode(sessionNode, [], 'Building Blocks', 'settings_24.png', 'Session');

            buildingBlockNodeNames = ["Task", "Parameters"];
            iconFileName = ["flask2.png", "param_edit_24.png"];
            ...
            for i = 1:numel(buildingBlockNodeNames)
                ...
                nodes = app.Session.Settings.(buildingBlockNodeNames(i));
            ...
            functionalityBlockNodeNames = ["Simulation", "Optimization", "CohortGeneration", "VirtualPopulationGeneration", "GlobalSensitivityAnalysis"];
            ...
            for i = 1:numel(functionalityBlockNodeNames)
                ...
                nodes = app.Session.(functionalityBlockNodeNames(i));
                for j = 1:numel(nodes)
        ...
        function constructToolstrip(app)
            ...
            openRecentItem.DynamicPopupFcn = @app.openRecentListFcn;
            ...
            % Import Dataset
            importSection = homeTab.addSection('Import');
            newColumn = importSection.addColumn();
            newColumn.add(matlab.ui.internal.toolstrip.Button('Import Dataset', 'src/+view/icons/import_24.png'));
            
            % Export
            exportSection = homeTab.addSection("Export");
            newColumn = exportSection.addColumn();
            exportButton = matlab.ui.internal.toolstrip.SplitButton('Export', 'src/+view/icons/export_24.png');
            newColumn.add(exportButton);
            popup = matlab.ui.internal.toolstrip.PopupList();
            simResultsItem = matlab.ui.internal.toolstrip.ListItem('Selected Simulations Results To Excel...');
            fittingResultsItem = matlab.ui.internal.toolstrip.ListItem('Data Fitting Results To Excel...');
            fittingSummaryToExcelItem = matlab.ui.internal.toolstrip.ListItem('Fitting Summary To PDF...');
            populationSimResultsToExcelItem = matlab.ui.internal.toolstrip.ListItem('Selected Population Simulation Results To Excel...');
            ncaToExcelItem = matlab.ui.internal.toolstrip.ListItem('Non-Compartmental Analysis (NCA) to Excel...');
            popup.add(simResultsItem);
            popup.add(fittingResultsItem);
            popup.add(fittingSummaryToExcelItem);
            popup.add(populationSimResultsToExcelItem);
            popup.add(ncaToExcelItem);
            exportButton.Popup = popup;
            
            % Plot Settings
            plotSettingsSection = homeTab.addSection("Plot Settings");
            newColumn = plotSettingsSection.addColumn();
            newColumn.add(matlab.ui.internal.toolstrip.Button('Plot Settings', 'src/+view/icons/plotSettings_24.png'));
    ...
    methods (Access = private)
        ...
        function create(app)
            ...
            % Create plugins menu
            app.PluginsMenu = uimenu(app.ToolsMenu);
            ...
            if ~isempty(Index)
                ...
                elseif ~isempty(Node.UserData)
                    ...
                    uimenu(...
                        ...
                        'Enable', 'off');
                    %TODO'MenuSelectedFcn', @(h,e) app.onDeleteSelectedItem(h.Parent.UserData.Parent.Parent.Parent.NodeData,h.Parent.UserData));
    ...
    methods (Access = private)
        ...
        function onTreeSelectionChanged(app, ~, event)
            ...
            % Store the panes (Today the ViewPane subclass in the UserData
            % of the TreeNode. If none there get one from the Pane type
            % list on the main view class.
            
            if length(event.SelectedNodes) == 1
                selectedNode = event.SelectedNodes;
                paneType = selectedNode.UserData;
                
                if isfield(app.paneHolder, paneType)
                    paneToActivate = app.paneHolder.(paneType);
                else
                    % We need to construct one.
                   panetoActivateFcn = app.getPaneClassFcnFromPaneType(paneType);
                   
                   paneToActivate = feval(panetoActivateFcn, {app.paneGridLayout, 1, 1, app});
                   
                   app.paneHolder(1).(paneType) = paneToActivate;                                      
                end
                
                % TODO come check this method call.
                f = eval("@attachNew"+paneType); % could avoid this by just doing what attach does here.
                assert(~isempty(selectedNode.NodeData));
                feval(f, paneToActivate, selectedNode.NodeData);               
                paneToActivate.show('Summary');
                
                if ~isempty(event.PreviousSelectedNodes)
                    app.paneHolder.(event.PreviousSelectedNodes.UserData).hide;
                end
            end
            return
    ...
    methods (Access = private)
        ...
        function restoreNode(app,node,session)
            try
                ...
                ItemType = app.ItemTypes{strcmpi(ItemClass,app.ItemTypes(:,2)),1};
    ...
    methods
        ...
        function set.SelectedSessionIdx(app,value)
            if isempty(value)
                ...
                validateattributes(value, {'double'},{'scalar','positive','integer','<=',app.NumSessions}); %TODO and Discuss
